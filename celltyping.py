...
ADATA_DIR = "/mnt/storage1/projects/research/24035_1345_TDP43/scanpy_analysis/adata_files"
...
FIGDIR = "/mnt/storage1/projects/research/24035_1345_TDP43/scanpy_analysis/figures_AB_mouse"
REPORTDIR = "/mnt/storage1/projects/research/24035_1345_TDP43/scanpy_analysis/reports"
...
def export_celltype_counts(adata, outdir, label_key="ct_label_mv", lowconf_label="Unknown_lowConf"):
    ...
    counts_all = (
        ...
        .rename_axis("cell_subtype")
        .reset_index(name="n_cells")
    )
    counts_all["percent"] = (100 * counts_all["n_cells"] / counts_all["n_cells"].sum()).round(3)
    counts_all.to_csv(os.path.join(outdir, "celltypist_counts_all_cells.tsv"), sep="\t", index=False)

    # filtered
    adata_filt = adata[adata.obs[label_key].astype(str) != lowconf_label].copy()
    counts_filt = (
        adata_filt.obs[label_key]
        .astype(str)
        .value_counts(dropna=False)
        .rename_axis("cell_subtype")
        .reset_index(name="n_cells")
    )
    counts_filt["percent"] = (100 * counts_filt["n_cells"] / counts_filt["n_cells"].sum()).round(3)
    counts_filt.to_csv(os.path.join(outdir, "celltypist_counts_filtered_noUnknownLowConf.tsv"), sep="\t", index=False)

    print("[IO] Saved cell type count tables:")
    print("   ", os.path.join(outdir, "celltypist_counts_all_cells.tsv"))
    print("   ", os.path.join(outdir, "celltypist_counts_filtered_noUnknownLowConf.tsv"))
...
def export_celltype_counts_subset(
    adata,
    mask,
    out_prefix,
    outdir,
    ...
    sub = adata[mask].copy()
    print(f"[Subset] {out_prefix}: {sub.n_obs:,} cells")
    ...
    counts_all["percent"] = (100 * counts_all["n_cells"] / counts_all["n_cells"].sum()).round(3)
    out_all = os.path.join(outdir, f"{out_prefix}_celltypist_counts_all_cells.tsv")
    counts_all.to_csv(out_all, sep="\t", index=False)

    # filtered
    sub_filt = sub[sub.obs[label_key].astype(str) != lowconf_label].copy()
    counts_filt = (
        sub_filt.obs[label_key]
        .astype(str)
        .value_counts(dropna=False)
        .rename_axis("cell_subtype")
        .reset_index(name="n_cells")
    ...
    counts_filt["percent"] = (100 * counts_filt["n_cells"] / counts_filt["n_cells"].sum()).round(3)
    out_filt = os.path.join(outdir, f"{out_prefix}_celltypist_counts_filtered_noUnknownLowConf.tsv")
    counts_filt.to_csv(out_filt, sep="\t", index=False)
...
def export_condition_comparison_table(
    adata,
    mask_vehicle,
    mask_tdp43,
    outdir,
    label_key="ct_label_mv",
    lowconf_label="Unknown_lowConf",
    prefix="Vehicle_vs_TDP43",
):
    os.makedirs(outdir, exist_ok=True)

    # all cells
    veh_all = (
        adata[mask_vehicle].obs[label_key]
        .astype(str)
        .value_counts(dropna=False)
        .rename("vehicle_n")
    )
    tdp_all = (
        adata[mask_tdp43].obs[label_key]
        .astype(str)
        .value_counts(dropna=False)
        .rename("tdp43_n")
    )

    comp_all = pd.concat([veh_all, tdp_all], axis=1).fillna(0)
    comp_all["vehicle_n"] = comp_all["vehicle_n"].astype(int)
    comp_all["tdp43_n"] = comp_all["tdp43_n"].astype(int)

    veh_total = comp_all["vehicle_n"].sum()
    tdp_total = comp_all["tdp43_n"].sum()

    comp_all["vehicle_percent"] = (100 * comp_all["vehicle_n"] / veh_total).round(3)
    comp_all["tdp43_percent"] = (100 * comp_all["tdp43_n"] / tdp_total).round(3)
    comp_all["delta_percent"] = (comp_all["tdp43_percent"] - comp_all["vehicle_percent"]).round(3)
    comp_all["log2FC_tdp43_vs_vehicle"] = np.log2((comp_all["tdp43_n"] + 1) / (comp_all["vehicle_n"] + 1)).round(4)

    comp_all = (
        comp_all
        .reset_index()
        .rename(columns={"index": "cell_subtype"})
        .sort_values("delta_percent")
    )

    out_all = os.path.join(outdir, f"{prefix}_all_cells.tsv")
    comp_all.to_csv(out_all, sep="\t", index=False)

    # filtered (exclude Unknown_lowConf)
    veh_filt = (
        adata[mask_vehicle].obs.loc[
            adata[mask_vehicle].obs[label_key].astype(str) != lowconf_label,
            label_key
        ]
        .astype(str)
        .value_counts(dropna=False)
    ...
    tdp_filt = (
        adata[mask_tdp43].obs.loc[
            adata[mask_tdp43].obs[label_key].astype(str) != lowconf_label,
            label_key
        ]
        .astype(str)
        .value_counts(dropna=False)
    ...
    comp_filt = pd.concat([veh_filt, tdp_filt], axis=1).fillna(0)
    comp_filt["vehicle_n"] = comp_filt["vehicle_n"].astype(int)
    comp_filt["tdp43_n"] = comp_filt["tdp43_n"].astype(int)

    veh_total_f = comp_filt["vehicle_n"].sum()
    tdp_total_f = comp_filt["tdp43_n"].sum()

    comp_filt["vehicle_percent"] = (100 * comp_filt["vehicle_n"] / veh_total_f).round(3)
    comp_filt["tdp43_percent"] = (100 * comp_filt["tdp43_n"] / tdp_total_f).round(3)
    comp_filt["delta_percent"] = (comp_filt["tdp43_percent"] - comp_filt["vehicle_percent"]).round(3)
    comp_filt["log2FC_tdp43_vs_vehicle"] = np.log2((comp_filt["tdp43_n"] + 1) / (comp_filt["vehicle_n"] + 1)).round(4)
    ...
    out_filt = os.path.join(outdir, f"{prefix}_filtered_noUnknownLowConf.tsv")
    comp_filt.to_csv(out_filt, sep="\t", index=False)

    print("[IO] Saved comparison tables:")
    print("   ", out_all)
    print("   ", out_filt)
...
def main():
    from celltypist import models
    m = models.Model.load("Mouse_Whole_Brain.pkl")
    print(m)
    sys.exit()
    ...
    #     print("[bc_wells examples]")
    #     print(adata.obs["bc_wells"].astype(str).head(20).tolist())
    # for col in ["sample", "bc1_well", "bc2_well", "bc3_well"]:
    #     if col in adata.obs.columns:
    #         print(f"\n[{col} unique examples]")
    #         vals = adata.obs[col].astype(str)
    #         print(vals.head(20).tolist())
    #         print("n_unique =", vals.nunique())
    #         print("first_unique =", sorted(vals.unique())[:30])
    # sys.exit()
    # Run celltypist
    ...
    # Condition split from bc1_well
    vehicle_wells = {f"A{i}" for i in range(1, 13)}  # A1-A12
    tdp43_wells = {f"B{i}" for i in range(1, 13)}  # B1-B12

    bc1 = adata.obs["bc1_well"].astype(str)

    mask_vehicle = bc1.isin(vehicle_wells)
    mask_tdp43 = bc1.isin(tdp43_wells)

    print("[Condition split]")
    print("Vehicle A1-A12 cells:", int(mask_vehicle.sum()))
    print("TDP-43 B1-B12 cells:", int(mask_tdp43.sum()))
    print("Total cells in A/B masks:", int((mask_vehicle | mask_tdp43).sum()))

    export_celltype_counts_subset(
        adata,
        mask=mask_vehicle,
        out_prefix="Vehicle_A1toA12",
        outdir=REPORTDIR,
        label_key="ct_label_mv",
        lowconf_label="Unknown_lowConf",
    )
    ...
    export_celltype_counts_subset(
        adata,
        mask=mask_tdp43,
        out_prefix="TDP43_B1toB12",
        outdir=REPORTDIR,
        label_key="ct_label_mv",
        lowconf_label="Unknown_lowConf",
    )
    export_celltype_counts(
        adata,
        outdir=REPORTDIR,
        label_key="ct_label_mv",
        lowconf_label="Unknown_lowConf",
    ...
    export_condition_comparison_table(
        adata,
        mask_vehicle=mask_vehicle,
        mask_tdp43=mask_tdp43,
        outdir=REPORTDIR,
        label_key="ct_label_mv",
        lowconf_label="Unknown_lowConf",
        prefix="Vehicle_A1toA12_vs_TDP43_B1toB12",
    )
