...
for org in orgs:
    ...
    if org == 'mouse':
        ...
        DATA = '/mnt/storage1/projects/research/24035_1345_TDP43/split_pipe_analysis_combined/one_sample_name/DGE_filtered/'
    elif org == 'rat':
        FIGDIR = '/mnt/storage1/projects/research/24035_1345_TDP43/scanpy_analysis/figures_CD_rat'
        DATA = '/mnt/storage1/projects/research/24035_1345_TDP43/split_pipe_analysis_combined_CD_rat/one_sample_name/DGE_filtered/'
    ...
    if USE_CELLTYPIST:
        ...
        adata_main.obs["ct_label"] = _as_series(
            getattr(pred_main, "predicted_labels", None),
            preferred="predicted_labels",
            index=adata_main.obs_names
        )

        mv_obj = getattr(pred_main, "majority_voting", None)
        if mv_obj is not None:
            adata_main.obs["ct_label_mv"] = _as_series(
                mv_obj,
                preferred="majority_voting",
                index=adata_main.obs_names
            )
        else:
            adata_main.obs["ct_label_mv"] = adata_main.obs["ct_label"]

        if hasattr(pred_main, "confidence") and pred_main.confidence is not None:
            adata_main.obs["ct_conf"] = _as_series(
                pred_main.confidence,
